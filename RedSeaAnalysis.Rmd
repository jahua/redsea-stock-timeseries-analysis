...
analyze_correlation_changes <- function(returns, crisis_start) {
  ...
  p <- ggplot(cor_changes_long, aes(x = Var1, y = Var2, fill = value)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          axis.title = element_blank()) +
    coord_fixed() +
    labs(title = "Changes in Correlation Structure During Crisis")
...
for (col in data$shipping_cols) {
  if (col %in% colnames(data$returns)) {
    ...
    by_month <- aggregate(asset_returns, 
                         format(index(asset_returns), "%Y-%m"), 
                         FUN = function(x) {
                           c(Mean = mean(x, na.rm = TRUE) * 100,
    ...
    month_df <- data.frame(
      Asset = col,
      Month = rownames(by_month),
      Mean = by_month$x[, "Mean"],
      Volatility = by_month$x[, "SD"],
      Min = by_month$x[, "Min"],
      Max = by_month$x[, "Max"],
      stringsAsFactors = FALSE
    )
...
for (i in 1:(ncol(data$returns)-1)) {
  for (j in (i+1):ncol(data$returns)) {
    ...
    pre_cor <- cor(data$returns[index(data$returns) < as.Date(data$crisis_start), asset1],
                  data$returns[index(data$returns) < as.Date(data$crisis_start), asset2],
                  use = "pairwise.complete.obs")
    
    crisis_cor <- cor(data$returns[index(data$returns) >= as.Date(data$crisis_start), asset1],
                     data$returns[index(data$returns) >= as.Date(data$crisis_start), asset2],
                     use = "pairwise.complete.obs")
    ...
    # Add to results
    significant_changes <- rbind(significant_changes, data.frame(
  ...
  for (asset in data$shipping_cols) {
    if (asset %in% colnames(data$returns)) {
      ...
      if (!is.null(bp_test) && !is.null(bp_test$breakpoints)) {
        # Get optimal number of breaks based on BIC
        summary_bp <- summary(bp_test)
        opt_breaks <- which.min(summary_bp$RSS / summary_bp$nobs + log(summary_bp$nobs) * 
                              summary_bp$npar / summary_bp$nobs)
        ...
        crisis_in_ci <- FALSE
        if (length(dates) > 0) {
          conf_int <- confint(bp_test, breaks = opt_breaks)
          crisis_date <- as.Date(data$crisis_start)
          
          for (i in 1:length(dates)) {
            lower_ci <- index(data$returns)[conf_int[i, 1]]
            upper_ci <- index(data$returns)[conf_int[i, 2]]
        ...
        bp_results <- rbind(bp_results, data.frame(
          ...
          BIC = summary_bp$BIC[opt_breaks + 1],
