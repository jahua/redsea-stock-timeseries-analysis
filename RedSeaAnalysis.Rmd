    ...
    if (asset %in% colnames(data$returns)) {
      ...
      if (length(returns_vector) >= 100) { # Need sufficient observations
        ...
        if (use_ewma_fallback || !model_converged) {
          ...
          # Get model properties - safely extract model type
          tryCatch({
            # Try to get information from the specification
            model_spec <- garch_result$spec
            
            # Determine model type based on the specification
            if (inherits(model_spec, "uGARCHspec")) {
              if (model_spec@model$varmodel == "gjrGARCH") {
                model_type <- "GJR-GARCH"
                garch_order <- model_spec@model$garchOrder
              } else if (model_spec@model$varmodel == "sGARCH") {
                # Check if it's ARCH (no GARCH term)
                garch_order <- model_spec@model$garchOrder
                if (garch_order[2] == 0) {
                  model_type <- "ARCH"
                } else {
                ...
                model_type <- model_spec@model$varmodel
                garch_order <- model_spec@model$garchOrder
              }
              # Get ARMA order and distribution model
              arma_order <- model_spec@model$armaOrder
              dist_model <- model_spec@model$distribution
            } else {
              # Fallback - extract from model name
              model_info <- garch_result$model@model
              model_type <- model_info$modeldesc$vmodel
              garch_order <- c(1, 1)  # Default fallback
              arma_order <- c(0, 0)   # Default fallback
              dist_model <- "norm"    # Default fallback
            }
          }, error = function(e) {
            # If any error occurs, use a safe fallback
            cat("Error extracting model info:", e$message, "\n")
            model_type <- "GARCH"
            garch_order <- c(1, 1)  # Default fallback
            arma_order <- c(0, 0)   # Default fallback
            dist_model <- "norm"    # Default fallback
          ...
          # Ensure model_type is defined - defensive approach
          if (!exists("model_type")) {
            model_type <- "GARCH"  # Default fallback
          }
          if (!exists("garch_order")) {
            garch_order <- c(1, 1)  # Default fallback
          }
          if (!exists("arma_order")) {
            arma_order <- c(0, 0)   # Default fallback
          }
          ...
          if (length(alpha_indices) > 0 && length(beta_indices) > 0) {
            ...
            garch_summary <- rbind(garch_summary, data.frame(
              ...
              Omega = if("omega" %in% names(model_coef)) model_coef["omega"] else NA,
              Alpha = sum(model_coef[alpha_indices]),
              Beta = sum(model_coef[beta_indices]),
              Persistence = persistence,
              AIC = if(!is.null(garch_result$model@fit$ics)) garch_result$model@fit$ics["Akaike"] else NA,
